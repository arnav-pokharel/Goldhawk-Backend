...
exports.getDiscountSafe = async (req, res) => {
  ...
  try {
    ...
    const raw = pick(row.discount_safe, row.DISCOUNT_SAFE);
...
// POST founder signature (write-once)
exports.signFounder = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT discount_safe FROM deal_safe WHERE deal_id = $1 LIMIT 1`,
      [deal_id]
    );
    if (result.rowCount === 0) return res.status(404).json({ error: 'SAFE not found' });
    let state = parseJsonDeep(result.rows[0].discount_safe || {});
    if (state.founderSignature?.signed) return res.status(409).json({ error: 'Founder already signed' });
    state.founderSignature = payload;
    await pool.query(
      `UPDATE deal_safe SET discount_safe = $2 WHERE deal_id = $1`,
      [deal_id, JSON.stringify(state)]
    );
    res.json({ success: true, founderSignature: safeParse(state.founderSignature) });
  } catch (err) {
    console.error('signFounder (discount) error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
};

// POST investor signature (write-once)
exports.signInvestor = async (req, res) => {
  ...
  try {
    ...
    console.error('signInvestor (discount) error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
};
