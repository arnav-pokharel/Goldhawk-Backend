...
exports.getProRataSafe = async (req, res) => {
  ...
  try {
    ...
    const raw = pick(row.pro_rata_side_letter_safe, row.PRO_RATA_SIDE_LETTER_SAFE);
    const payload = parseJsonDeep(raw || {});

    // Back-compat: some rows store a bare signature object at the root
    // Map that to the founder's signature so investors only need to sign.
...
exports.signFounder = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT pro_rata_side_letter_safe FROM deal_safe WHERE deal_id = $1 LIMIT 1`,
      [deal_id]
    );
    if (result.rowCount === 0) return res.status(404).json({ error: 'SAFE not found' });

    let state = parseJsonDeep(result.rows[0].pro_rata_side_letter_safe || {});

    if (state.founderSignature?.signed)
      return res.status(409).json({ error: 'Founder already signed' });

    state.founderSignature = payload;

    await pool.query(
      `UPDATE deal_safe SET pro_rata_side_letter_safe = $2 WHERE deal_id = $1`,
      [deal_id, JSON.stringify(state)]
    );

    res.json({ success: true, founderSignature: safeParse(state.founderSignature) });
  } catch (err) {
    console.error('signFounder (prorata) error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
exports.signInvestor = async (req, res) => {
  ...
  try {
    ...
    if (state.investorSignature?.signed)
    ...
    console.error('signInvestor (prorata) error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
};
